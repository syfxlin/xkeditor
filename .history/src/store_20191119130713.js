...
const store = {
  ...
  actions: {
    operateModal(operate, isShow, title = '') {
      ...
      store.state.aceToolbarModal.content = operate;
      store.state.aceToolbarModal.data.modalTitle = title;
      store.state.aceToolbarModal.show = true;
    },
    aceToolbarSubmit() {},
    aceToolbarCancer() {
      store.state.aceToolbarModal.content = '';
      store.state.aceToolbarModal.show = false;
    },
    imgUpload() {},
    graffUpload() {
  ...
  if (states instanceof Array) {
    ...
    for (const key in states) {
      if (states[key] instanceof Function) {
        ret[states[key]] = states[key](store.state);
      } else {
        ret[states[key]] = store.state[states[key]];
      }
    }
  }
  return ret;
