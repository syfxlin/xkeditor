...
export const mapState = states => {
  ...
  if (states instanceof Array) {
    for (let i = 0; i < states.length; i++) {
      ret[states[i]] = state[states[i]];
    }
  } else {
    for (const key in states) {
      if (states[key] instanceof Function) {
        ret[key] = states[key](state);
      } else {
        ret[key] = state[states[key]];
      }
    }
  }
  return ret;
...
export const mapActions = actions => {
  ...
  if (actions instanceof Array) {
    for (let i = 0; i < actions.length; i++) {
      ret[actions[i]] = actions[actions[i]];
    }
  } else {
    for (const key in actions) {
      if (actions[key] instanceof Function) {
        ret[key] = actions[key](actions);
      } else {
        ret[key] = actions[actions[key]];
      }
    }
  }
  return ret;
